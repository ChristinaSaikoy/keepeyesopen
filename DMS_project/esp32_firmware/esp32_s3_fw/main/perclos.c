...
void perclos_mark_observation_invalid(uint32_t now_ms, perclos_state_t *state)
{
    ...
    state->yawn_confirmed = false;
    state->mouth_closed_active = false;
    state->classifier_eyes_closed = false;
    state->classifier_yawning = false;
    state->should_send_alert = false;
    set_normal(state);
}
...
void perclos_update_classifier(float face_valid_probability,
                               float eye_closed_probability,
                               float yawn_probability,
                               uint32_t now_ms,
                               perclos_state_t *state)
{
    dms_fatigue_observation_t observation = {0};
    if (state == NULL) {
        return;
    }
    if (!isfinite(face_valid_probability) || !isfinite(eye_closed_probability) || !isfinite(yawn_probability) ||
        face_valid_probability < 0.0f || face_valid_probability > 1.0f || eye_closed_probability < 0.0f ||
        eye_closed_probability > 1.0f || yawn_probability < 0.0f || yawn_probability > 1.0f ||
        face_valid_probability < CLASSIFIER_FACE_VALID_THRESHOLD) {
        perclos_mark_observation_invalid(now_ms, state);
        return;
    }
    state->classifier_eyes_closed = state->classifier_eyes_closed ?
                                        eye_closed_probability > CLASSIFIER_EYE_CLOSED_RECOVER_THRESHOLD :
                                        eye_closed_probability >= CLASSIFIER_EYE_CLOSED_START_THRESHOLD;
    state->classifier_yawning = state->classifier_yawning ?
                                    yawn_probability > CLASSIFIER_YAWN_RECOVER_THRESHOLD :
                                    yawn_probability >= CLASSIFIER_YAWN_START_THRESHOLD;
    observation.valid = true;
    observation.eyes_closed = state->classifier_eyes_closed;
    observation.yawning = state->classifier_yawning;
    perclos_update_semantic(&observation, now_ms, state);
}
